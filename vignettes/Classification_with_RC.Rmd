...
```{r}
model_fit <- fit(node = model,
                 X = X_train[[1]],
                 Y = Y_train[[1]],
                 stateful=FALSE,
                 warmup = 2)

Y_pred <- predict_seq(node = model_fit, 
                      X = X_test[[1]],
                      stateful = FALSE)
Y_pred
```
...
Y_test_class <- sapply(Y_test[[1]], FUN = function(x) apply(as.matrix(x),1,which.max))
score <- accuracy(Y_pred_class, Y_test_class)
...
k <- 1
for (x in X_train) {
  states <- predict_seq(node = reservoir, X = x, reset=TRUE)
  states_train[[k]] <- states[[length(states)]]
  k <- k+1
}
```
...
for (x in X_train) {
  states <- predict_seq(node = reservoir, X = x, reset=TRUE)
  y <- predict_seq(node = readout, X = states[[length(states)]])
  Y_pred[[k]] <- y
  k+1
}
```
...
score <- accuracy(pred = as.factor(unlist(Y_test_class)),
                       truth = as.factor(unlist(Y_pred_class)))
