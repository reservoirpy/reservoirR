...
Y_pred <- reservoir::predict_seq(node = model_fit$fit,
                                 X = X_test,
                                 stateful = FALSE)
```
...
k <- 1
for (x in X_train) {
  states <- reservoir::predict_seq(node = reservoir, X = x, reset=TRUE)
...
```{r}
res <- reservoir::reservoirR_fit(readout,X = states_train, Y = Y_train)
summary(res)
```
...
for (x in X_test) {
  states <- reservoir::predict_seq(node = reservoir, X = x, reset=TRUE)
  y <- reservoir::predict_seq(node = readout, X = as.array(states[nrow(states),]))
  Y_pred[[k]] <- y
  k <- k+1
}
```
